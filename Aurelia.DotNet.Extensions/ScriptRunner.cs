...
namespace Microsoft.AspNetCore.NodeServices
{
    ...
    public class ScriptRunner
    {
        ...
        {
            // When the package manager task emits complete lines, pass them through to the real logger
            StdOut.OnReceivedLine += line =>
            {
                if (!string.IsNullOrWhiteSpace(line))
                {
                    // package manager tasks commonly emit ANSI colors, but it wouldn't make sense to forward
        ...
        private static Process LaunchNodeProcess(ProcessStartInfo startInfo, string packageManager)
        {
            try
            {
            ...
            {
                var message = $"Failed to start '{packageManager}'. To resolve this:.\n\n"
                            + $"[1] Ensure that '{packageManager}' is installed and can be found in one of the PATH directories.\n"
